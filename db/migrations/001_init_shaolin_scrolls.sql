...
CREATE TABLE IF NOT EXISTS dojo.release_type (
...
-- Seed values (safe to re-run)
INSERT INTO dojo.release_status (code) VALUES
...
CREATE TABLE IF NOT EXISTS dojo.shaolin_scrolls (
  ...
  -- Status (lookup)
  release_status_id SMALLINT NOT NULL REFERENCES dojo.release_status(id),

  -- Added: release_type nuance (lookup)
  release_type_id   SMALLINT NOT NULL REFERENCES dojo.release_type(id),
...
DROP TRIGGER IF EXISTS trg_audit_release_type ON dojo.release_type;
CREATE TRIGGER trg_audit_release_type
BEFORE INSERT OR UPDATE ON dojo.release_type
FOR EACH ROW
EXECUTE FUNCTION dojo.audit_stamp_generic();

DROP TRIGGER IF EXISTS trg_audit_release_status ON dojo.release_status;
CREATE TRIGGER trg_audit_release_status
BEFORE INSERT OR UPDATE ON dojo.release_status
FOR EACH ROW
EXECUTE FUNCTION dojo.audit_stamp_generic();

DROP TRIGGER IF EXISTS trg_audit_shaolin_scrolls ON dojo.shaolin_scrolls;
CREATE TRIGGER trg_audit_shaolin_scrolls
BEFORE INSERT OR UPDATE ON dojo.shaolin_scrolls
FOR EACH ROW
EXECUTE FUNCTION dojo.audit_stamp_generic();
