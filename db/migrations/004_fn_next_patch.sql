...
-- Purpose: rename fn_next_hotfix to fn_next_patch with status/release_type.
-- Verification:
--   psql $DATABASE_URL -f db/migrations/004_fn_next_patch.sql
--   psql $DATABASE_URL -c "SELECT * FROM dojo.fn_next_patch('Label','planned','hotfix');"
...
begin
  select rs.id into v_status_id from dojo.release_status rs where rs.code = p_status_code;
  if v_status_id is null then raise exception 'Unknown release_status code: %', p_status_code; end if;

  select rt.id into v_release_type_id from dojo.release_type rt where rt.code = p_release_type_code;
  if v_release_type_id is null then raise exception 'Unknown release_type code: %', p_release_type_code; end if;
