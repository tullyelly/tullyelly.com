...
export async function GET(req: Request) {
  try {
    ...
    return NextResponse.json({ items, page } satisfies ReleaseListResponse, {
      headers: {
    ...
    console.error("shaolin-scrolls list error", err);
    return NextResponse.json({ error: "internal error" }, { status: 500 });
  }
}

export type { ReleaseRow, ReleaseListResponse, PageMeta } from "@/lib/scrolls";
