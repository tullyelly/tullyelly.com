...
export function ScrollsTable({
  ...
  const table = useReactTable({
    data: memoData,
    columns,
    // Ensure stable row IDs based on immutable primary key
    getRowId: (row) => row.id,
    state: { globalFilter },
                ...
                {hg.headers.map(h => {
                  ...
                  const ariaSort =
                    sortState === 'asc'
                      ...
                      ? 'descending'
                      : 'none';
                  const headerText = String(h.column.columnDef.header);
                  const nextSort = sortState === 'asc' ? 'descending' : 'ascending';
                  return (
                    <th
                      key={h.id}
                      scope="col"
                    ...
                    >
                      {h.isPlaceholder ? null : (
                        <button
                          ...
                          aria-label={`Sort by ${headerText} ${nextSort}`}
                        >
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoading ? (
              Array.from({ length: pageSize }).map((_, i) => (
                ...
                <tr key={`skeleton-${i}`} className="odd:bg-white even:bg-[#EEE1C6] hover:bg-[#0077C0]/10">
                  {columns.map((col, idx) => (
              ...
              table.getRowModel().rows.map(r => (
                <tr key={r.original.id} className="odd:bg-white even:bg-[#EEE1C6] hover:bg-[#0077C0]/10">
