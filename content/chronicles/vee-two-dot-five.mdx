...
There's a few places on the site that I don't want anyone rummaging around in, so I linked up to the big Google in the sky and integrated some of their front-end login functionality with my own authz controls on the backend. Integrating with Google was surprisingly easy, and if you want to go on an Easter Egg hunt, you could find the page to activate the Google Machine. Let me know if you find it - that's fun.
