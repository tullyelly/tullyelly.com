...
export default function PersonaReleaseFeed({ persona }: { persona: Persona }) {
  ...
  return (
    <section
      className="space-y-4"
      aria-labelledby={`${persona}-latest-releases`}
    >
      <div className="flex flex-wrap items-baseline justify-between gap-3">
        <h1
          id={`${persona}-latest-releases`}
          className="text-xl font-semibold leading-snug md:text-2xl"
        >
          latest releases
        </h1>
        <Link
          href={getPersonaReleaseLogHref(persona)}
          className="link-blue text-sm md:text-base"
        >
          all {config.label} →
        </Link>
      </div>
          ...
          {entries.map((entry) => (
            ...
            >
              ...
              <p className="mt-2 text-[15px] leading-relaxed text-muted-foreground">
                {releaseSectionPreview(entry.bodyMdx) ||
                  "Open the chronicle to view this release."}
              </p>
              <Link
                href={getReleaseSectionHref(
                  entry.postUrl,
                ...
                className="mt-2 inline-block text-sm link-blue"
                aria-label={`Open original chronicle: ${entry.postTitle}`}
              >
