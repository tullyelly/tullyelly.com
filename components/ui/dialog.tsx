...
export const Content = React.forwardRef<HTMLDivElement, ContentProps>(
  (
    ...
    const startRef = React.useRef<{ x: number; y: number; ox: number; oy: number } | null>(null);
    const halfSizeRef = React.useRef<{ halfW: number; halfH: number }>({ halfW: 0, halfH: 0 });
    ...
    const onPointerDown = React.useCallback((e: React.PointerEvent<HTMLDivElement>) => {
      ...
      const interactive = target.closest('button, [role="button"], a, input, textarea, select');
    ...
    React.useEffect(() => {
      ...
      const observer = new MutationObserver(() => {
        const isHidden = node.hasAttribute("hidden") || node.getAttribute("aria-hidden") === "true" || node.getAttribute("data-state") === "closed";
        if (!isHidden) {
      ...
      observer.observe(node, { attributes: true, attributeFilter: ["hidden", "aria-hidden", "data-state"] });
