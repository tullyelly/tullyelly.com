...
import type { LcsNarrativeDay, LcsSection } from "@/lib/lcs-content";
...
type RenderableSection = LcsSection & { code: string };
type RenderableDay = LcsNarrativeDay & {
...
function compareVisitDaysAsc(
  a: Pick<LcsNarrativeDay, "visitDate">,
  b: Pick<LcsNarrativeDay, "visitDate">,
): number {
  return toTimestamp(a.visitDate) - toTimestamp(b.visitDate);
}

function getSourcePostLabel(
  post: LcsNarrativeDay["sourcePosts"][number],
): string {
  return post.title ?? fmtDate(post.date);
}

export default async function LcsChronicleFeed({
  days,
  entryLabel,
  emptyMessage,
  missingContentMessage,
}: LcsChronicleFeedProps) {
  ...
  if (renderableDays.length === 0) {
    return (
      <p className="text-sm leading-6 text-[color:var(--lcs-ink)]/80">
        {emptyMessage}
      </p>
    );
  }
      ...
      {renderableDays.map((day, dayIndex) => (
        <div key={day.visitDate} className="space-y-0">
          {dayIndex > 0 ? (
            <div
              aria-hidden="true"
              className="my-8 h-[4px] w-full rounded bg-[color:var(--lcs-accent)]"
            />
          ) : null}
          ...
          >
            <header className="flex flex-wrap items-center gap-x-4 gap-y-2">
              <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2">
                <div className="inline-flex min-h-[2.25rem] items-center whitespace-nowrap text-sm font-semibold leading-none text-[color:var(--lcs-ink)]">
                  ...
                  >
                    {day.sourcePosts.map((post) => (
                      <Link
                        ...
                        href={post.url}
                        className="inline-flex min-h-[2.25rem] items-center whitespace-nowrap font-medium leading-none text-[color:var(--lcs-link)] transition hover:text-[color:var(--lcs-link-hover)]"
                      >
              ...
              <span className="ml-auto inline-flex min-h-[2.25rem] items-center rounded-full bg-[color:var(--lcs-accent)] px-3 py-1 text-sm font-semibold text-[color:var(--lcs-pill-fg)] shadow-sm">
                {`${entryLabel} ${dayIndex + 1}`}
              </span>
            </header>

            <div className="rounded-[28px] border border-[color:var(--lcs-border)] bg-[color:var(--lcs-surface)] px-4 py-4 shadow-sm md:px-5 md:py-5">
                ...
                {day.compiledSections.length > 0 ? (
                  day.compiledSections.map((section, sectionIndex) => (
                    <div
                      ...
                      className={
                        sectionIndex > 0
                          ? "border-t border-[color:var(--lcs-border)] pt-8"
                          : ""
                      }
                    >
