...
import type { SavePointSection } from "@/lib/save-point";

type SavePointSectionsProps = {
  sections: SavePointSection[];
};

type RenderableSection = SavePointSection & { code: string };
...
export default async function SavePointSections({
  sections,
}: SavePointSectionsProps) {
  ...
  for (const section of compiledSections) {
    ...
    sectionEntries.push({
      section,
      anchorId: `review-${section.postSlug}-${next}`,
      key: `${baseKey}-${next}`,
    });
  }
      ...
      {hasMultipleSections ? (
        <div className="flex flex-wrap gap-3 text-sm">
          {sectionEntries.map(({ anchorId, key }, index) => (
            <Link key={`${key}-jump`} href={`#${anchorId}`} className="link-blue">
              {`Jump to Review ${index + 1}`}
            </Link>
          ))}
        </div>
      ) : null}
      ...
      {sectionEntries.map(({ section, anchorId, key }, index) => (
        <section key={key} id={anchorId} className="space-y-4">
          <h2 className="text-xl md:text-2xl font-semibold leading-tight">
            {fmtDate(section.postDate)}: Review {index + 1}{" "}
