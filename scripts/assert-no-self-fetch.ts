...
const ROOTS = ['app', 'lib'];
const SERVER_GLOBS = ['page.tsx', 'page.ts', 'layout.tsx', 'layout.ts', '.ts', '.tsx'];
...
for (const f of files) {
  ...
  if (/getBaseUrl\(\).*\/api\//.test(s)) offenders.push(f + ' -> getBaseUrl() + /api');
}

if (offenders.length) {
  console.error('❌ Found intra-app fetches in server files:\n' + offenders.join('\n'));
  process.exit(1);
} else {
